...
---comment
---@param kind "press"|"release"
---@param hand 0|1
---@param button ENUM_DIGITAL_INPUT_ACTIONS
---@param callback function
function Input:RegisterCallback(kind, hand, button, callback)
    local b = tracked_buttons[button]
    local h = b[hand]
    if h then
        ...
        for handid, data in pairs(hands) do
            ...
            if player:IsDigitalActionOnForHand(hand:GetLiteralHandType(), button) then
                ...
                if data.press_time == -1 then
                    ...
                    data.release_time = -1
                    -- print(Input:GetButtonDescription(button), "pressed")
                    local send = {
                        kind = "press",
